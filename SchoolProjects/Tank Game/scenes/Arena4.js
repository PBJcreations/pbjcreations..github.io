...
class Arena4 extends Scene {
  constructor() {
    super("Arena 4"); //The name of our scene
  }
  start() {
    this.camera = new Camera(20, "gray");// Add a camera game object
    ...
    {//Obstacles / inner walls
      this.instantiate(Prefabs.getPrefabByName("medRSQ"), new Vector2(2, 2), Math.PI/4);
      this.instantiate(Prefabs.getPrefabByName("medRSQ"), new Vector2(-2, -2), Math.PI/4);
      this.instantiate(Prefabs.getPrefabByName("mediumSQ"), new Vector2(-5, 5), Math.PI/4);
      this.instantiate(Prefabs.getPrefabByName("mediumSQ"), new Vector2(5, -5), Math.PI/4);

      this.instantiate(Prefabs.getPrefabByName("medRSQ"), new Vector2(-4, -4), Math.PI/3);
      this.instantiate(Prefabs.getPrefabByName("medRSQ"), new Vector2(4, 4), Math.PI/3);
      this.instantiate(Prefabs.getPrefabByName("smallSQ"), new Vector2(-6, 2), Math.PI/3);
      this.instantiate(Prefabs.getPrefabByName("smallSQ"), new Vector2(6, -2), Math.PI/3);

      this.instantiate(Prefabs.getPrefabByName("medRSQ"), new Vector2(-8, -8), Math.PI/6);
      this.instantiate(Prefabs.getPrefabByName("medRSQ"), new Vector2(8, 8), Math.PI/6);
      this.instantiate(Prefabs.getPrefabByName("medRSQ"), new Vector2(2, -6), Math.PI/6);
      this.instantiate(Prefabs.getPrefabByName("medRSQ"), new Vector2(-2, 6), Math.PI/6);
